...
def chu_cun(c):
    ...
    for i in range(l):
        ...
        runp = p.add_run('')
        runp.add_picture('T.jpg', width=Inches(3.5), height=Inches(2.1875))
        gs2 = t2.add_run(str2)
        gs2.font.name = 'Times New Roman'
        gs2.font.element.rPr.rFonts.set(qn('w:eastAsia'), '新宋体')
        gs2.font.size = Pt(12.5)
        if i + 1 != l:
            gs2.add_break(docx.enum.text.WD_BREAK.PAGE)
    os.remove('T.jpg')
    try:
        os.remove('推荐.docx')
    except:
...
def pdf_hua():
    ...
    pdfpath = os.path.join(path, '推荐.pdf')
